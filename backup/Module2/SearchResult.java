        ...
        try {
            ...
            for(int i=0; i<expectedTableBody.size();i++){
                ...
                for(int j=0;j<expectedTableRow.size();j++){
                    ...
                    String expectedTableBodyText = expectedTableRow.get(j);
                    int row=i+1;
                    int column=j+1;
                    String tableBodyXpath = "//table/tbody/tr[" + row + "]/td[" + column + "]";
                    WebElement tableBodyElement = driver.findElement(By.xpath(tableBodyXpath));
                    String actualTableBodyContent = tableBodyElement.getText();
                    if(!actualTableBodyContent.equals(expectedTableBodyText)){
                        status = false;
                    }
