...
public class Home {
    ...
    public Boolean addProductToCart(String productName) {
        try {
             ...
             */
            List<WebElement> titleElemments = driver.findElements(
                    By.xpath("//p[@class='MuiTypography-root MuiTypography-body1 css-yg30e6']"));
            List<WebElement> addToCartElemments =
                    driver.findElements(By.xpath("//button[text()='Add to cart']"));

            for (int i = 0; i < titleElemments.size(); i++) {
                WebElement title = titleElemments.get(i);
                String actualProductName = title.getText();
                if (actualProductName.equals(productName)) {
                    WebElement addToCart = addToCartElemments.get(i);
                    addToCart.click();
                    Thread.sleep(3000);
                    return true;
                }
    ...
    public Boolean changeProductQuantityinCart(String productName, int quantity) {
        try {
            ...
            List<WebElement> productParentElements =
                    driver.findElements(By.xpath("//div[@class='MuiBox-root css-1gjj37g']"));
            for (WebElement parentElement : productParentElements) {
